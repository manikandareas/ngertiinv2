...
export function SessionPage() {
  ...
  const orders = useMemo(
    () => (bundle?.questions ?? []).map((q) => q.questionOrder).sort((a, b) => a - b),
    [bundle?.questions],
  );
  useEffect(() => {
  ...
  const optionsByQuestion: OptionsByQuestion =
    (bundle?.optionsByQuestion ?? {}) as OptionsByQuestion;
  ...
  const { mutate: saveAnswer, isPending: saving } = useMutation({
    mutationFn: useConvexMutation(saveAnswerFn),
    onSuccess: () => {
      setSavedAt(Date.now());
      // Scoped invalidation of session bundle only (if needed)
  ...
  const { mutate: submitSession, isPending: submitting } = useMutation({
    ...
    onSuccess: () => {
      // Invalidate both session and result bundles
      qc.invalidateQueries({ queryKey: sessionBundleKey });
  ...
  const gotoOrder = useCallback(
    (order: number) => {
      ...
      const nextOrder = exists
        ? order
        : orders.reduce(
            (prev, curr) => (Math.abs(curr - order) < Math.abs(prev - order) ? curr : prev),
            base,
          );
  ...
  }, [sessionId, submitSession, unansweredOrders]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (!Number.isNaN(num) && num >= 1 && num <= 9) {
        ...
        if (opt) {
          e.preventDefault();
          saveAnswer({ sessionId, questionId: q._id, selectedOptionId: opt._id });
        }
      }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [saving, submitting, orders, activeOrder, gotoOrder, handleSubmitClick, questions, optionsByQuestion, saveAnswer, sessionId]);
  ...
  const selectedOptionId = qDoc ? answers[qDoc._id] : undefined;
  ...
  const now = Date.now();
  const endTimePassed = Boolean(bundle.lab.endTime && now > (bundle.lab.endTime ?? 0));
  const attemptsLeft =
    bundle.lab.maxAttempts != null
      ? Math.max((bundle.lab.maxAttempts ?? 0) - (bundle.session.attemptNumber ?? 0), 0)
      : undefined;
    ...
    <div className="mx-auto w-full max-w-5xl px-6 py-8 space-y-6">
      ...
      {endTimePassed ? (
        <div className="rounded-md border border-amber-200 bg-amber-50 text-amber-900 p-3 text-sm">
          Periode pengerjaan telah berakhir. Attempt ini dimulai sebelum end time dan masih dapat diselesaikan.
        </div>
      ) : null}
      {attemptsLeft === 0 ? (
        <div className="rounded-md border border-blue-200 bg-blue-50 text-blue-900 p-3 text-sm">
          Batas attempt tercapai. Anda tidak dapat memulai attempt baru setelah ini.
        </div>
      ) : null}
      ...
      <NavigationBar
        ...
        reviewPanel={
          <ReviewPanel
            total={totalQuestions}
            answeredOrders={questions.filter((q) => answers[q._id]).map((q) => q.questionOrder)}
