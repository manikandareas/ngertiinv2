...
export const hardestQuestions = query({
	...
	handler: async (ctx, { labId, limit, windowDays }) => {
		...
		for (const q of questions) {
			...
			const inWindow = answers.filter(
				(a) => (a.answeredAt ?? 0) >= windowStart,
			);
			if (inWindow.length === 0) continue;

			const attempts = inWindow.length;
			const corrects = inWindow.reduce(
		...
		const enriched = stats
			...
			.sort(
				(a, b) => a.accuracy - b.accuracy || a.questionOrder - b.questionOrder,
			);
...
export const timePerQuestion = query({
	...
	handler: async (ctx, { labId, limit, windowDays }) => {
		...
		rows.sort(
			(a, b) => b.medianMs - a.medianMs || a.questionOrder - b.questionOrder,
		);
