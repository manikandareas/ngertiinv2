	...
	handler: async (ctx, args) => {
		...
		await Promise.all(
			args.orderedIds.map((id, index) =>
				ctx.db.patch(id, { questionOrder: index }),
			),
		);
...
export const updateQuestion = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (typeof args.questionText !== "undefined")
			patch.questionText = args.questionText;
		if (typeof args.explanation !== "undefined")
			patch.explanation = args.explanation;
